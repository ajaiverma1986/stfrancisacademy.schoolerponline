...
public partial class user_login : System.Web.UI.Page
{
    ...
    private void LoginCheck(string uid, string pas, string Type)
    {
        ...
        if (DtForOtpOdp.Rows.Count > 0)
        {
            ...
            {
                if (GetIpAddress() != Convert.ToString(DtForOtpOdp.Rows[0]["IpAdressForLogin"]))
                {
                ...
                else
                {
                    ...
                    {
                        if (!Convert.ToString(DtForOtpOdp.Rows[0]["MacAdressForLogin"]).Contains(GetMACAddress()))
                        {
    ...
    {
        string str = "SELECT ISNULL(RID,0) RID,[UserID],[FirstName],[MiddleName],[LastName],[MobNo],[ParentTypeID],[ParentName] " +
                     " ,[Fyid],[Brid],[GenderID],[dob],[UserName],[Password] FROM [student_login] " +
                     " where UserName='" + txtUserName.Text.Trim() + "' and password ='" + txtPassword.Text.Trim() + "'";
        DataTable dt = objut.GetDataTable(str);
        if (dt.Rows.Count > 0)
        {
            Session["UserName"] = dt.Rows[0]["UserName"].ToString();
            Session["UserBrid"] = dt.Rows[0]["Brid"].ToString();
            Session["UserNewID"] = dt.Rows[0]["UserID"].ToString();
            Session["UserRID"] = dt.Rows[0]["RID"].ToString();

            Response.Redirect("StudentPanel/RegExam.aspx", false);
        }
        else {
            Utility.ViewAlertMessage(this, "Invalid UserName and Password");
        }
