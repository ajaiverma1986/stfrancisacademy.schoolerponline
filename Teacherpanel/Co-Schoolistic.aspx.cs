...
public partial class Teacherpanel_Co_Schoolistic : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString.Count == 1)
                {
                    ...
                    //Query = "objDutselect classid,classname from exam_class where examid=" + eid + " and status=" + 1 + "";
            ...
            if (examid > 0)
            {
                ...
                Query = " Select Distinct cm.id as classid,cm.Classname from classwithsection cws " +
                    ...
                    //" inner join exam_class ec on ec.brid = cws.branchid " +
                         " where cm.status=1  and cm.id in (select tc.classid from tbl_addTeacherToClass tc where tc.status=1 and tc.empid=" + EmpId + ") and cws.branchid=" + Session["brBrid"] + " and cm.fyid=" + fyid; //+ "" +
