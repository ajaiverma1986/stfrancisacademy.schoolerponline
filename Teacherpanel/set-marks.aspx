...
<asp:Content ID="Content1" ContentPlaceHolderID="head" Runat="Server">  
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $("#<%=grdsetmarks.ClientID%> input[id*='chkstatus']:checkbox").click(function () {
                var isAbsent = 0;
                var row = $(this).closest("tr");

                var regno = $("[id*=lblregno]", row).text();
                var currentmarks = $("[id*=txtmarks]", row).val();
                var maxmarks = $("[id*=hfmaxmarks]", row).val();

                var chkIsAbsent = $("[id*=chkstatus]:checked", row);
                if (chkIsAbsent.length > 0)
                    isAbsent = 1;

                saveMarks(isAbsent, regno, currentmarks, maxmarks);
            });

            $("input[id*=txtmarks]").on('blur paste', function () {
                debugger
               
                var isAbsent = 0;
                var row = $(this).closest("tr");
                var regno = $("[id*=lblregno]", row).text();
                var currentmarks = $(this).val();
                var maxmarks = $("[id*=hfmaxmarks]", row).val();
        ...
        function saveMarks(isAbsent, regno, currentmarks, maxmarks) {
            debugger

            if (isString($.trim(currentmarks)))
                saveValue(isAbsent, regno, currentmarks);
            else {
                currentmarks = parseInt($.trim(currentmarks));
        ...
        function saveValue(isAbsent, regno, currentmarks) {
            debugger
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "set-marks.aspx/SaveOrUpdateMark",
                data: "{'regNo':'" + regno + "','subjectId':'" + $('#ctl00_head_ddlsubject').val() + "','examId':'" + $('#ctl00_head_ddlexam').val() + "','marks':'" + currentmarks + "','classId':'" + $('#ctl00_head_ddlcourse').val() + "','sectionId':'" + $('#ctl00_head_ddlsection').val() + "','IsAbsent':'" + isAbsent + "'}",
                dataType: "json",
