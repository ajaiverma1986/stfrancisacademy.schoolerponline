...
public partial class UserReg : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        idStudent = Convert.ToInt32(objdut.GetScalar("select isnull(max(stuRegNo),0)+1 from tbl_studentMaster where brid=" + Convert.ToInt32(ddlBrid.SelectedValue)));
        ...
        if (fname.Length > 3)
        {
            tempFname = fname.ToString().Substring(0, 3);
        }
        else {
            tempFname = fname.ToString();
        }
        UserName = tempFname.ToString() + idStudent.ToString() + ddlBrid.SelectedValue.ToString() + ddlregistrationsession.SelectedValue.ToString();
        ...
        param[14] = new SqlParameter("@ClasssID", ddlapplyingforclass.SelectedValue);
        objdut.ExecuteSqlSP(param, "Usp_NewReg");

        count = Convert.ToInt32(param[13].Value);
        ...
        if (count == 1)
        {
            ...
            if (!ReturnValue())
            {
                string strconfirm = "<script>if(!window.confirm('" + msg + "')){window.location.href='user-login.aspx'}else{window.location.href='user-login.aspx'}</script>";
