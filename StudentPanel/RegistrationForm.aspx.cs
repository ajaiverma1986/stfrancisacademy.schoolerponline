...
public partial class StudentPanel_RegistrationForm : System.Web.UI.Page
{
    ...
    private void getrecieptgenraldetails()
    {
        ...
        string branchdata = Convert.ToString(objdut.GetScalar(@"select BrAddress+','+City+' - '+pincode+', '+state+', '+Country+'^'+
            LandLineNo+','+MobileNo+'^'+Emailid+'^'+isnull(Website,'')+'^'+isnull(SchoolAffi,'') from branch_master where brid = '" + Session["UserBrid"] + "'"));
    ...
    protected void Receipt(string RegNo)
    {
        ...
        using (SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"].ToString()))
        {
            using (SqlCommand cmd = new SqlCommand("[usp_studentregistrationHistory]", con))
            {
                ...
                param[0] = new SqlParameter("@Brid", System.Web.HttpContext.Current.Session["UserBrid"].ToString());
