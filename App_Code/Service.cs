...
public class Service : System.Web.Services.WebService
{
    ...
    public string OnlineTransFeeCol(string Account, string BankName, string TransID, string FromDate, string ToDate, string Name, string MobNo, string ReciptNo)
    {
        ...
        Param[4] = new SqlParameter("@MobileNo", MobNo);
        Param[5] = new SqlParameter("@ReciptNo", ReciptNo);
        Param[6] = new SqlParameter("@Account", Account);
        Param[7] = new SqlParameter("@BankName", BankName);
        Param[8] = new SqlParameter("@TransID", TransID);

        var transDetailDT = Objdut.GetDataSetSP(Param, "USP_TransportFee");
        return transDetailDT.GetXml().ToString();
    }
        ...
        using (SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"].ToString()))
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
            {
                ...
                cmd.CommandText = @"select DISTINCT Case when PrimaryEmailid = '' then SecondaryEmailId Else PrimaryEmailid End as studentEmailID,studentFirstName+' '+isnull(studentMiddleName,'')+' '+isnull(studentLastName,'') as name
                               ,convert(varchar(12),studentdob,105)as dob,isnull(PrimaryParentFirstName,'')+' '+isnull(PrimaryParentMiddleName,'')+' '+isnull(PrimaryParentLastName,'') father,
                         ...
                         (select Brname from Branch_master where Brid=@brid) as CollegeName,
                          (select top 1 FYName from financial_Year where isactive=1) as sessionYear,
                            ...
                            from tbl_studentregistration inner join Class_Master on tbl_studentregistration.ApplyingForClass = Class_Master.id where rid=@eid and brid=@brid) as className,classwithsection.sectionname
                                ,tbl_StudentRegistration.Applyingforclass as ClassID from tbl_StudentRegistration 
                                inner join tbl_StudentMaster on tbl_StudentRegistration.RID = tbl_StudentMaster.RID
                ...
                if (Rd.Read())
                {
                    ...
                    Obj.classID = Convert.ToString(Rd["ClassID"]);
    ...
    [WebMethod]
    [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
    public string BindTransAmount(string Brid,string Fyid,string SturegNo)
    {
        DataSet ds = new DataSet();
        using (SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"].ToString()))
        {
            using (SqlCommand cmd = new SqlCommand("[usp_StudentTransFee]", con))
            {
                ...
                param[0] = new SqlParameter("@Mode", Convert.ToInt32(8));
                ...
                param[10] = new SqlParameter("@Fyid", Fyid.ToString());
                ...
                param[16] = new SqlParameter("@StuRegNo", SturegNo.ToString());
                cmd.Parameters.Add(param[0]);
                cmd.Parameters.Add(param[1]);
                ...
                //            if (dtrow["PayStatus"].ToString() == "1")
                //            {
                //                user.Status = "Unpaid";
                //            }
                //            else if (dtrow["PayStatus"].ToString() == "2")
                //            {
                //                user.Status = "Paid";
                //            }
                //            details.Add(user);
