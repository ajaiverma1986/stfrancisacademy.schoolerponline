...
public partial class EmployeePanel_employee_view_profile : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static offEmpList[] GetAppointedEmpList()
    {
        ...
        p[1] = new SqlParameter("@pagesize", SqlDbType.Int);
        p[1].Value = 10;

        p[2] = new SqlParameter("@pageno", SqlDbType.Int);
        p[2].Value = 1;

        p[3] = new SqlParameter("@eid", SqlDbType.VarChar, 100);
        p[3].Value = HttpContext.Current.Session["eid"].ToString();

        p[4] = new SqlParameter("@status", SqlDbType.VarChar, 50);
        p[4].Value = "1";
        ...
        foreach (DataRow dtrow in ds.Tables[0].Rows)
        {
            ...
            DataTable dtlogin = ObjDUT.GetDataTable("select username,password from Employee_login where status=1 and eid=" + Convert.ToInt32(HttpContext.Current.Session["eid"]) + "");
