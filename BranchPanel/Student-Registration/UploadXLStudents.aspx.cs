                ...
                if (tblStudentDtl.Rows.Count > 0)
                {
                    ...
                    for (int j = 0; j < tblStudentDtl.Rows.Count; j++)
                    {
                        if (Convert.ToString(tblStudentDtl.Rows[j]["First_Name"]) != "")
                        {
                            ...
                            if (tblStudentDtl.Rows[j]["PrimaryParentDOB"].ToString() != "")
                            {
                            ...
                            {
                                RegistrationDate = (Convert.ToDateTime(tblStudentDtl.Rows[j]["AdmissionDate"]).ToString("yyyy-MM-dd")).ToString();
                            }
                            else
						    {
                    ...
                    // else wala condition data upload ke liye hai
					      //{  
...
//                                }
                ...
                {
                    Utility.ViewAlertMessage(this, "Some Student registration not successfully done. Please Check Student Managemnet Module to Verify");
                }
            }
            else
            {
