...
public partial class BranchPanel_SectionAdd : System.Web.UI.Page
{
    ...
    private void fillgrid()
    {
        string querynew = @"select cwsid,classid,(Cm.Classname)As classname,sectionname,Case when Cws.status=1 then 'Active' else 'Not Active' end as Status from
                   ClassWithSection Cws inner join Class_Master Cm ON Cws.Classid=Cm.Id  where branchid=" + Session["BrBrid"] + " order by classid asc,cwsid asc";
    ...
    protected void griview_section_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName == "Edit1")
        {
            ...
            divall.Visible = true;
            Button2.Visible = false;
            string str = "select classid,ClassName,sectionname,cws.status from classwithsection cws inner join Class_Master cm on cm.id=cws.classid  where branchid=" + Session["brBrid"] + "and cwsid=" + asd;
