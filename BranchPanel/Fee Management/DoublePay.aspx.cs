 ...
 */
        ...
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString.Count > 0)
            {
                ...
                if (Request.QueryString[0] != null)
                {
                    ...
                    //if (ReceiptT.Length < 2)
                    //{
                    //    ID = ReceiptT[0].ToString();
                    //}
                    //else
                    //{
                    //    StrQuery = "Select ISNULL(ASAID,0) ASAID from Receipt_master where Receiptno=" + ReceiptT[1] + " and Brid=" + Convert.ToInt32(Session["BrBrid"]) + "";
    ...
    protected void Receipt(int id)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            //Fyid = Convert.ToInt32(dt.Rows[0]["Fyid"]);
var fyidName = Convert.ToString(Objdut.GetScalar(@"Select ISNULL(FYName,'') from tbl_StudentAdmissionInstallment sf
                                            INNER JOIN financial_Year fy on sf.Fyid = fy.FYID
                                            where sf.ASAID = " + id + ""));
             spSession1.InnerHtml = fyidName;
			 spSession.InnerHtml = fyidName;
            var value = Convert.ToDateTime(dt.Rows[0]["PaidDate"]).ToString("dd MMM yyyy HH:mm tt");
            ...
            recieptdate.InnerHtml = value; // dt.Rows[0]["PaidDate"].ToString();
            recieptdate2.InnerHtml = value; //dt.Rows[0]["PaidDate"].ToString();
            ...
            //if (dt.Rows[0]["PaymentModeName"].ToString().ToUpper() == "CASH" || dt.Rows[0]["PaymentModeName"].ToString().ToUpper() == "CARD")
            if (dt.Rows[0]["PaymentModeName"].ToString() == "case" || dt.Rows[0]["PaymentModeName"].ToString() == "Cash" || dt.Rows[0]["PaymentModeName"].ToString() == "Card")
            //	if (dt.Rows[0]["PaymentModeName"].ToString() == "Cash" || dt.Rows[0]["PaymentModeName"].ToString() == "Card")if (dt.Rows[0]["PaymentModeName"].ToString().ToUpper() == "CASH" || dt.Rows[0]["PaymentModeName"].ToString().ToUpper() == "CARD")

            {
                var paymentMode = "Cash";

                if (dt.Rows[0]["PaymentModeName"].ToString() == "Card")
                {
                    paymentMode = dt.Rows[0]["PaymentModeName"].ToString();
                }

                Span7.InnerHtml = paymentMode.ToString();

                Span56.InnerHtml = paymentMode.ToString();
            ...
            //else if(dt.Rows[0]["PaymentModeName"].ToString() == "QR Code")
            {
                chequedetails.Visible = false;
            ...
            int Truncate1 = (int)(Objdut.ExecuteSql("TRUNCATE TABLE tblTemp"));
            ...
            if (dtAsaid.Rows.Count > 0)
            {
                ...
                if (dtFeeDetails.Rows.Count > 0)
                {
                    ...
                    string strPaid = "Select PaidDate,ISNULL(PaidNonTaxableAmount,0) AMOUNT from tbl_StudentAdmissionInstallment " +
                                     " where Brid=" + Session["BrBrid"] + " and SturegNo=" + dt.Rows[0]["SturegNo"].ToString() + "" +
                                     " and MonthId IN(" + MonthID + ") and PaymentStatusId=2 and ASAID <> " + id + " and Fyid=" + Fyid;
    ...
    private void getClassSection(int id)
    {
        if (Session["BrBrid"] != null)
        {
            var brid = Convert.ToInt32(Session["BrBrid"]);
            if (brid > 0)
            {
                string strQuery = @"Select distinct SturegNo,Fyid,Brid from tbl_StudentAdmissionInstallment where ASAID=" + id + " and Brid=" + brid + "";
                DataTable dt = objdut.GetDataTable(strQuery);
                if (dt.Rows.Count > 0)
                {
                    string strQueryClassSec = @"select cm.Classname,(Case When IsNull(sectionname,'')='' Then 'A' Else sectionname End) As sectionname 
                                                from tbl_StudentAdmissionMaster SA 
                                                inner join Class_Master cm on cm.Id=SA.ClassID          
                                                left join classwithsection cws on cws.cwsid=SA.SectionId  
                                                where SA.StuRegNo=" + dt.Rows[0]["SturegNo"].ToString() + " and SA.Fyid=" + dt.Rows[0]["Fyid"].ToString() + " and SA.Brid=" + brid
                                                + @" UNION
                                                select cm.Classname,(Case When IsNull(sectionname,'')='' Then 'A' Else sectionname End) As sectionname 
                                                from Adm_StudentPromoteDtl SA 
                                                inner join Class_Master cm on cm.Id=SA.CurrentClassID          
                                                left join classwithsection cws on cws.cwsid=SA.CurrentSectionID  
                                                where SA.StuRegNo=" + dt.Rows[0]["SturegNo"].ToString() + " and SA.PreFyid=" + dt.Rows[0]["Fyid"].ToString() + " and SA.Brid=" + brid;

                    DataTable dtClassSect = objdut.GetDataTable(strQueryClassSec);
                    if (dtClassSect.Rows.Count > 0)
                    {
                        Span3.InnerHtml = dtClassSect.Rows[0]["Classname"].ToString();
                        Span43.InnerHtml = dtClassSect.Rows[0]["Classname"].ToString();
                        ...
                        Span4.InnerHtml = dtClassSect.Rows[0]["sectionname"].ToString();
                        Span44.InnerHtml = dtClassSect.Rows[0]["sectionname"].ToString();
                    }
                }
            }
        }
