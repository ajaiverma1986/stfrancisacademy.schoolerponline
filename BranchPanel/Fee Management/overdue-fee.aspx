<%@ Page Title="" Language="C#" MasterPageFile="~/BranchPanel/MasterPage.master" EnableEventValidation="false" AutoEventWireup="true" CodeFile="overdue-fee.aspx.cs" Inherits="BranchPanel_Fee_Management_overdue_fee" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script>
        function printDiv() {
            document.getElementById("asdsmaster").style.display = "none";
   ...
   style = document.createElement('style');
            ...
            window.print();
            document.getElementById("asdsmaster").style.display = "block";
        }
    </script>
    <div id="MyDialog">       
    </div>
        ...
        <div class="content_pane_cont input_content" id="contentPane">
                ...
                <div class="upload">
				 ...
				 <div class="upload">
                    ...
                    <asp:Button ID="Button1" runat="server" Text="Month Wise Report" Visible="true"
                        CausesValidation="false" Height="20px" PostBackUrl="~/BranchPanel/Fee Management/overdue-Monthwise.aspx" Style="float: right;"
                        ToolTip="Click here to download Excel sheet" />
                </div>
                <div class="upload">
                    <input type="button" class="btn-info" id="btnprint" value="Print PDF" onclick="printDiv()"/>
                </div>
            </div>
            <div class="conductedlecture234 firstbox" style="border: 1px solid #DEB1D9;">
                <dd style="color:black;">Total Students</dd>
                <div class="clear"></div>
                <span id="lbltotalstudent" style="font-weight: bold;color:black;"></span>
            </div>
            <div class="conductedlecture234 secbox" style="border: 1px solid #f5d88c;">
                <dd style="color:black;">Total Overdue Fees</dd>
                <div class="clear"></div>
                <span id="lbloverduefee" style="font-weight: bold;color:black;"></span>
            </div>
             <div class="conductedlecture234 secbox" style="border: 1px solid #f5d88c;height:49px;padding-top:15px">
                 <a href="StudentWithFee.aspx" style="text-decoration:underline;cursor:pointer">Enrolled Student list</a>
                  
                
            </div>
            <div class="conductedlecture234 secbox" style="border: 1px solid #f5d88c;height:49px;padding-top:15px">
                 <a href="StudentWithoutFee.aspx" style="text-decoration:underline;cursor:pointer">UnEnrolled Student list</a>
                 
            </div>
            <div class="conductedlecture234 secbox" style="border: 1px solid #f5d88c;height:49px;padding-top:15px">
                <asp:FileUpload ID="fileUpload" runat="server" />
                <asp:Button runat="server" ID="btnSaveExcel" Text="Upload" OnClick="btnUpload_Click" />
                 <asp:Label ID="lblError" runat="server" ForeColor="Red"></asp:Label> <asp:Label ID="lblmessage" runat="server" ForeColor="Green"></asp:Label>
            </div>
			<div class="conductedlecture234 secbox" style="border: 1px solid #f5d88c;height:49px;padding-top:15px">
                 <a href="EnrolledStudents.aspx" style="text-decoration:underline;cursor:pointer">Enrolled Students</a>
            ...
            <div class="grid-data-resposive">
                <div class="searchbar containerHeadline ">
                    ...
                    <input class="Allsendremainder" type="image" style="height:25px;width:25px;border-width:0px;" alt="No Image" src="../../images/message.png">
                    <input class="SendAllToken" type="image" style="height:25px;width:25px;border-width:0px;" alt="No Image" src="../../images/Token.png">
                    <input class="SendAllToken1" type="button" style="height:25px;width:60px;" value="Token" />
                    <div class="clear"></div>
                </div>
                <div class="divTableDataHolder m-l-sm m-r-sm">
                    <div class="divforbindgrid">
                        <table class="stm m-t-sm" style="width: 100%;" cellpadding="10" cellspacing="0">
                            <tr class="stm_head" align="center">
                                <th align="center" scope="col">S.No.</th>
                                <th align="center" scope="col">Reg No No</th>
                    ...
                    <tr>
                        ...
                        <div style="margin-left: 0px; float: left; width: 500px;height:500px" class="page-number">
                  ...
                  <td>      <div style="margin-left: 0px; float: left; width: 500px; height:500px">
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            $('.ImgBtnForPayNow').live('change', function () {
                ...
                asaid = $(this).parent('td').parent('tr').find('td:eq(11)').find('input[type="hidden"]').val();
                $('#ctl00_ContentPlaceHolder1_hfforasaid').val(asaid);
                return false;
            });
            $('.SendAllToken').live('click', function () {
                if ($("#ckAll").is(':checked')) {
                    ckAll = 'checked';
                }
                else {
                ...
                var columns, k = 0, AsaidList = '', StdRegList = '';
                for (var i = 1; i < rows.length; i++) {
                    columns = $(rows[i]).find('td');
                    var j = 11, k = 2;
                    var ALLSAID = new Array();
                    var ALLSTDREG = new Array();
                    ALLSAID = $(columns[j]).html().substring(28, 50).split('"');
                    ALLSTDREG = $(columns[k]).html().substring(28, 50).split('"');

                    if (columns.find('input[type="checkbox"]').is(':checked')) {
                        AsaidList = AsaidList + ',' + ALLSAID[0];
                        StdRegList = StdRegList + ',' + ALLSTDREG[0];
                        k++;
                    } else {
                        k--;
                    }
                ...
                if (k == 0) {
                    ...
                    SendToken(AsaidList, StdRegList)
                }
                return false;
            });

            $('.SendAllToken1').live('click', function () {
                if ($("#ckAll").is(':checked')) {
                    ckAll = 'checked';
                }
                else {
                ...
                if (k == 0) {
                    ...
                    SendToken1(AsaidList, StdRegList)
                }
                return false;
            });
            ...
            $('.sendToken,.commentimg').live('click', function () {
                trindexforpay = $(this).parent('td').parent('tr').index();
                var stdRegNo = $(this).parent('td').parent('tr').find('td:eq(2)').find('input[type="hidden"]').val();
                asaid = $(this).parent('td').parent('tr').find('td:eq(11)').find('input[type="hidden"]').val();
                $('#ctl00_ContentPlaceHolder1_hfforasaid').val(asaid);
                SendToken(asaid, stdRegNo);
                return false;
            });
            
        });

        function SendToken(asaid, stdRegNo) {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "overdue-fee.aspx/SendToken",
                data: "{'ASAID':'" + asaid + "', 'stdRegNo':'" + stdRegNo + "'}",
                dataType: "json",
                success: function (data) {
                    if (data.d) {
                        openPopup('PrintToken.aspx', 550, 920, 'Over Due Token');
                        return false;
                    }
                    else {
                        alert('Msg not Send');
        ...
        function SendToken1(asaid, stdRegNo) {
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "overdue-fee.aspx/SendToken",
                ...
                success: function (data) {
                    if (data.d) {
                        openPopup('PrintToken1.aspx', 550, 1100, 'Over Due Token');
                        return false;
                    }
                    else {
                        alert('Msg not Send');
        ...
        function openPopup(url, h, w, t) {
            if (url != null && h != null && w != null && t != null) {
                ...
                url = urlBase + url;
                $('#MyDialog').html('<iframe id="print-iframe" name="print-frame-name" src="' + url +'" border=0 width="100%" height ="88%" src="' + url + '"> </iframe>' +
                        ...
                        for (var i = 0; i < data.d.length; i++) {
                            if (i % 2 != 0)
                                trforappend = '<tr class="stm_light"><td>' + data.d[i].Rownumber + '</td>' +
                                    ...
                                    '<td><input type="hidden" value="' + data.d[i].Asaid + '" /><input   class="sendToken"  type="image"  style="height:25px;width:25px;border-width:0px;" alt="No Image" src="../../images/Token.png"></td>' +
