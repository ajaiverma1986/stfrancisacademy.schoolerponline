...
public partial class BranchPanel_Fee_Management_PrintToken : System.Web.UI.Page
{
    ...
    protected void grdSTDResult_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
			...
			HtmlGenericControl lblAdnumber = (HtmlGenericControl)e.Row.FindControl("lblAdnumber");
            ...
            if (dfregno.Value != null)
            {
                ...
                BindPersonalInfo(lblFName, lblClass,SpanSec,lblAdnumber, lblStdName, Convert.ToInt32(dfregno.Value));
    ...
    private void BindFee(HtmlGenericControl spMName, HtmlGenericControl spBalance, string ASAID, HtmlTableCell tdFeedetails)
    {
        ...
        if (dtFee.Rows.Count > 0) {
            var html = "<table style='width:100%'><tr style='font-weight: 800;'><td>Month Name</td><td>Fee Name</td><td>Balance Amount</td></tr>";
            ...
            html = html + "<tr><td>&nbsp;&nbsp;&nbsp;&nbsp;</td><td>Total Amount:</td><td>" + total.ToString() + "</td><tr/>";
            html = html + "</table>";
    ...
    private void BindPersonalInfo(HtmlGenericControl lblFName, HtmlGenericControl lblClass,HtmlGenericControl SpanSec,HtmlGenericControl lblAdnumber, HtmlGenericControl lblStdName, int eid)
    {
        ...
        if (dtPerInfo.Rows.Count > 0)
        {
			...
			//sectionname = Convert.ToInt32(dtPerInfo.Rows[0]["sectionname"];
			...
			//SpanSec.InnerText = dtPerInfo.Rows[0]["sectionID"].ToString();
			SpanSec.InnerHtml = sectionID;
			lblAdnumber.InnerText = dtPerInfo.Rows[0]["RegNewNo"].ToString();
