...
public partial class BranchPanel_Fee_Management_LYFeeReceipt : System.Web.UI.Page
{
    ...
    protected void Receipt(string RegNo)
    {
        ...
        using (SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"].ToString()))
        {
            using (SqlCommand cmd = new SqlCommand("[usp_StudentLastYearFee]", con))
            {
                ...
                param[1] = new SqlParameter("@RID", enrollid.ToString());
                ...
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        ...
                        string SectionID = "0";
                        string PaidAmount = "0";
                        PaidAmount = objdut.GetScalar("Select ISNULL(FeeAmount,0) from LastYearPayDT where PayAmtID=" + enrollid + "").ToString();
                        recipetrecivername.InnerText = ds.Tables[0].Rows[0]["StudentFirstName"].ToString();
                        Span2.InnerText = ds.Tables[0].Rows[0]["Registration_No"].ToString();
                        Span5.InnerText = PaidAmount.ToString();      
                        ClassID = ds.Tables[0].Rows[0]["ApplyingForClass"].ToString();
                        ...
                        //}
                        recieptdate.InnerHtml = Convert.ToDateTime(ds.Tables[0].Rows[0]["RegistrationDate"]).ToString("dd MMM yyyy");
                    }
                }
            }
        }
