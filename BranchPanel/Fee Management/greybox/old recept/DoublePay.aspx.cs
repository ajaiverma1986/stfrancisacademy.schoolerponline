 ...
 */
    ...
    protected void Receipt(int id)
    {
        ...
        if (dtAsaid.Rows.Count > 0)
        {
            ...
            {
                //commented to solve fee details issue on reciept(13 july 19)
                ...
                string MonthID = "";
                decimal TransPort = 0;
                decimal FeeFineAmt = 0;
                decimal HostelFee = 0;
                //string str = dtFeeDetails.Compute("Sum(BalAmt)", "srno=1 or srno in(1,2)").ToString();
               
                for (int k = 0; k < dtFeeDetails.Rows.Count; k++)
                {
                    if (Convert.ToDecimal(dtFeeDetails.Rows[k]["TransPort"]) != 0)
                    {
                        TransPort = TransPort + Convert.ToDecimal(dtFeeDetails.Rows[k]["TransPort"]);
                    }
                    if (Convert.ToDecimal(dtFeeDetails.Rows[k]["FeeFine"]) != 0)
                    {
                        FeeFineAmt = FeeFineAmt + Convert.ToDecimal(dtFeeDetails.Rows[k]["FeeFine"]);
                    }
                    if (Convert.ToDecimal(dtFeeDetails.Rows[k]["HostelFee"]) != 0)
                    {
                        HostelFee = HostelFee + Convert.ToDecimal(dtFeeDetails.Rows[k]["HostelFee"]);
                    }
                ...
                if (TransPort != 0)
                {
                    ...
                    dr["SNo"] = Convert.ToInt32(dtFeeHead.Rows.Count) + 1;
                    dr["FeeHead"] = "Transport Fee";
                    dr["FeeHeadAmt"] = TransPort;
                    dtFeeHead.Rows.Add(dr);
                    total = total + TransPort;
                }
                if (Convert.ToString(dtAsaid.Rows[0]["FeeFineAmt"])!="" && Convert.ToDecimal(dtAsaid.Rows[0]["FeeFineAmt"]) != 0)
                {
                    DataRow dr = dtFeeHead.NewRow();
                    dr["SNo"] = Convert.ToInt32(dtFeeHead.Rows.Count) + 1;
                    dr["FeeHead"] = "Late Fee";
