...
public partial class BranchPanel_Fee_Management_StudentWithoutFee : System.Web.UI.Page
{
    ...
    public static GetOverDueFeeDetails[] BindGrid(string ClassId, string sectionId)
    {
        ...
        string query= "select distinct m.StuRegno,c.Classname,cs.sectionname,isnull(r.StudentFirstName,'')+' '+ isnull(r.StudentMiddleName,'')+' '+isnull(r.StudentLastName,'') as Name " +
        " , isnull(r.PrimaryParentFirstName, '')+' ' + isnull(r.PrimaryParentMiddleName, '') + ' ' + isnull(r.PrimaryParentLastName, '') as ParentName,r.PrimaryMobileNo from tbl_StudentMaster m"+
        " inner join tbl_StudentRegistration r on m.Registration_No = r.Registration_No"+
        " inner join Class_Master c on m.Classid=c.id inner join classwithsection cs on m.sectionid=cs.cwsid " +
        " where m.SturegNo not in (select SturegNo from Receipt_master where PaymentDate between '2019-01-01' and '2020-03-31')  and m.IsTerminated=0 and m.IsCompleted=0 and m.BrId=" + Convert.ToString(HttpContext.Current.Session["Brbrid"]) + " and r.FyId=" + Convert.ToString(ActiveFinancialyear); ;
