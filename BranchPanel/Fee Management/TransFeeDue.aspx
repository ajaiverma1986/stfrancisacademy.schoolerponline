<%@ Page Title="" Language="C#" MasterPageFile="~/BranchPanel/MasterPage.master" AutoEventWireup="true" CodeFile="TransFeeDue.aspx.cs" Inherits="BranchPanel_Fee_Management_TransFeeDue" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
                    ...
                    <div class="divforbindgrid">
                        <table class="stm m-t-sm" style="width: 100%;" cellpadding="0" cellspacing="0">
                            <tr class="stm_head" align="center">
                                ...
                                <th align="center" scope="col">Bus No</th>    
                              <%--   <th align="center" scope="col">Month Name</th>        
    ...
    <script type="text/javascript">
        var Fyid = 0, pagesize = 1000, pageno = 1, sectionID = 0, trindexforpay = 0, pagecount = 0, stuname = '', duefromdate = '';
        var duetodate = '', mobileno = '', classid = 0, year = 2050;
        ...
        function BindGridAfterSearch() {
            ...
            $.ajax({
                type: "POST",
                contentType: "application/json; charset=utf-8",
                url: "TransFeeDue.aspx/BindGridOfOverDueList",
                ...
                success: function (data) {
                    if (data.d.length > 0) {
                            ...
                            if (i % 2 != 0)
                                trforappend = '<tr class="stm_light"><td>' + (i + 1) + '</td>' +
                                    ...
                                    '<td><input type="hidden" value="' + data.d[i].Sturegno + '" /><input type="hidden" value="' + data.d[i].Samid + '" /><input type="hidden" value="' + data.d[i].Asaid + '" /><span class="StudentName">' + data.d[i].Name + '</span></td>' +
									 '<td><input type="hidden" value="' + data.d[i].Sturegno + '" /><input type="hidden" value="' + data.d[i].Samid + '" /><input type="hidden" value="' + data.d[i].Asaid + '" /><span class="FatherName">' + data.d[i].FName + '</span></td>' +
                                    '<td><span>' + data.d[i].classes + '</span></td>' +
                                    '<td>' + data.d[i].SectionID + '</td>' +
                                    '<td>' + data.d[i].PayableOrDueAmount + '</td>' +
                                    '<td>' + data.d[i].Mobile + '</td>'+
                                    '<td>' + data.d[i].Vehiclename + '</td>';
                                 //   '<td>' + data.d[i].MonthsName + '</td></tr>';
                        ...
                        trforappend = '<tr class="stm_light"><td></td>' +
                                   '<td></td>' +
                                   '<td></td>' +
                                   '<td></td>' +
                                   '<td>' +
                                   '<td>Total Due Amount</td>' +
                                   '<td>' + data.d[0].TotalOverDueFee + '</td>' +
                                   '<td></td></tr>';
                        $('.stm').append(trforappend);
                    }
                    else {
