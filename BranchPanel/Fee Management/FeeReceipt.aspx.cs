...
public partial class BranchPanel_Fee_Management_FeeReceipt : System.Web.UI.Page
{
    ...
    protected void Receipt(int id)
    {
        try { string RegistrationNo = "";
            ...
            if (dtAsaid.Rows.Count > 0)
            {
                ...
                {
                    for (int a = 0; a < dtFeeDetails.Rows.Count; a++)
                    {
                        if (Convert.ToDecimal(dtFeeDetails.Rows[a]["TransPort"]) != 0)
                        {
                            DataRow dr = dtFeeHead.NewRow();
                            dr["SNo"] = Convert.ToInt32(dtFeeHead.Rows.Count) + 1;
                            dr["FeeHead"] = "Transport Fee";
                            dr["FeeHeadAmt"] = Convert.ToDecimal(dtFeeDetails.Rows[a]["TransPort"]);
                            dtFeeHead.Rows.Add(dr);
                            total = total + Convert.ToDecimal(dr["FeeHeadAmt"]);
                        }

                        if (Convert.ToDecimal(dtFeeDetails.Rows[a]["FeeFine"]) != 0)
                        {
                            DataRow dr = dtFeeHead.NewRow();
                            dr["SNo"] = Convert.ToInt32(dtFeeHead.Rows.Count) + 1;
                            dr["FeeHead"] = "Late Fee";
                            dr["FeeHeadAmt"] = Convert.ToDecimal(dtFeeDetails.Rows[a]["FeeFine"]);
                            dtFeeHead.Rows.Add(dr);
                            total = total + Convert.ToDecimal(dr["FeeHeadAmt"]);
                        }

                        if (Convert.ToDecimal(dtFeeDetails.Rows[a]["HostelFee"]) != 0)
                        {
                            DataRow dr = dtFeeHead.NewRow();
                            dr["SNo"] = Convert.ToInt32(dtFeeHead.Rows.Count) + 1;
                            dr["FeeHead"] = "Hostel Fee";
                            dr["FeeHeadAmt"] = Convert.ToDecimal(dtFeeDetails.Rows[a]["HostelFee"]);
                            dtFeeHead.Rows.Add(dr);
                            total = total + Convert.ToDecimal(dr["FeeHeadAmt"]);
                        }
                    }
