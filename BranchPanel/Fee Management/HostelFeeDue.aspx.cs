...
public partial class BranchPanel_Fee_Management_HostelFeeDue : System.Web.UI.Page
{
    ...
    public static GetOverDueFeeDetails[] BindGridOfOverDueList(int SectionID, int Fyid, string pagesize, string pageno, string stuname, string duefromdate, string duetodate, string mobileno, string classid)
    {
        ...
        Ds = Objdut.GetDataSetSP(param, "[Usp_BindHostelDue]");
        if (Ds.Tables[0].Rows.Count > 0)
        {
            foreach (DataRow Dtrow in Ds.Tables[0].Rows)
            {
                ...
                user.DueDate = Dtrow["MonthN"].ToString();
                user.PayableOrDueAmount = Dtrow["FeeAmount"].ToString();
                ...
                user.TotalDue = Ds.Tables[0].Compute("SUM(FeeAmount)", "Sturegno=" + Dtrow["Sturegno"].ToString() + "").ToString();
