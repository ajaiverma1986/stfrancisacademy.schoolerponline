...
public partial class BranchPanel_Exams_ReportCard_Term : System.Web.UI.Page
{
        ...
        if (dtExam.Rows.Count > mm)
        {
            ...
            if (dtSubject.Rows.Count > 0)
            {
                ...
                if (dtMarks.Rows.Count > 0)
                {
                    ...
                    int total1 = 0, total2 = 0, total3 = 0, total4 = 0, total5 = 0, total6 = 0, finalPercentage = 0;
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        if (i == 7)
                        {
                            ...
                            finalPercentage = ((total3 + total6) * 100) / (subjectCount * 200);
                    ...
                    try
                    {
                        rank = ObjDut.GetScalar("select _rank from (select studentregno,sum(marks) as totalMarks, DENSE_RANK () OVER (  ORDER BY sum(marks) DESC  ) _rank  from tbl_marks_master where classid=" + ClassID + " and fyid=" + fyid + " and brid=" + Convert.ToInt32(Session["BrBrid"]) + "   group by studentregno ) t where t.studentregno='" + eid + "'").ToString();
                    }
                    catch (Exception ex)
                    {
                    ...
                    try
                    {
                        ranksection = ObjDut.GetScalar("select _rank from (select studentregno,sum(marks) as totalMarks, DENSE_RANK () OVER (  ORDER BY sum(marks) DESC  ) _rank  from tbl_marks_master where examid in(1,2) and classid=" + ClassID + " and sectionid=" + SectionID + " and fyid=" + fyid + " and brid=" + Convert.ToInt32(Session["BrBrid"]) + "   group by studentregno ) t where t.studentregno='" + eid + "'").ToString();
                        ranksection1 = ObjDut.GetScalar("select _rank from (select studentregno,sum(marks) as totalMarks, DENSE_RANK () OVER (  ORDER BY sum(marks) DESC  ) _rank  from tbl_marks_master where examid in(3,4) and classid=" + ClassID + " and sectionid=" + SectionID + " and fyid=" + fyid + " and brid=" + Convert.ToInt32(Session["BrBrid"]) + "   group by studentregno ) t where t.studentregno='" + eid + "'").ToString();
                        finalrank = ObjDut.GetScalar("select _rank from (select studentregno,sum(marks) as totalMarks, DENSE_RANK () OVER (  ORDER BY sum(marks) DESC  ) _rank  from tbl_marks_master where classid=" + ClassID + " and sectionid=" + SectionID + " and fyid=" + fyid + " and brid=" + Convert.ToInt32(Session["BrBrid"]) + "   group by studentregno ) t where t.studentregno='" + eid + "'").ToString();
                    }
                    catch (Exception ex)
                    {
    ...
    private string getGrade(decimal TotalMarks, string SubjectName)
    {
        ...
        if (SubjectName.ToLower() == "general knowledge" || SubjectName.ToLower() == "drawing" || SubjectName.ToLower() == "mv")
        {
            ...
            else if (TotalMarks > 99 && TotalMarks < 130)
            {
                Grade = "C";
            }
            else if (TotalMarks > 130 && TotalMarks < 160)
            {
                Grade = "B";
            }
            else if (TotalMarks > 160&& TotalMarks <=200)
            {
                Grade = "A";
            }
