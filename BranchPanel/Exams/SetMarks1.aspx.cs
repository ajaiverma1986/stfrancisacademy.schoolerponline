...
public partial class BranchPanel_Exams_SetMarks : System.Web.UI.Page
{
    ...
    protected void grdsetmarks_RowDataBound1(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (ddlsubject.SelectedValue != "0")
            {
                string strSubMark = "";
                if (IsGradeType)
                    strSubMark = "ISnull(Grade,'') IsGrade";
                else
                    strSubMark = "ISnull(marks,'0') marks";

                string strMarks1 = "Select "+ strSubMark + " from tbl_marks_master where sectionid=" + ddlsection.SelectedValue + " and examid=" + TempDt.Rows[Start1][0] + " and classid=" + ddlcourse.SelectedValue + " and subjectid=" + ddlsubject.SelectedValue + " and studentregno=" + lblregno.Text.ToString() + "";

                Marks1 = Convert.ToString(objDut.GetScalar(strMarks1));

                if (Marks1 == "" && IsGradeType == false)
                {
                    Marks1 = "0";
                }
                ...
                string strMarks2 = "Select "+ strSubMark + " from tbl_marks_master where sectionid=" + ddlsection.SelectedValue + " and examid=" + TempDt.Rows[Start2][0] + " and classid=" + ddlcourse.SelectedValue + " and subjectid=" + ddlsubject.SelectedValue + " and studentregno=" + lblregno.Text.ToString() + "";

                Marks2 = Convert.ToString(objDut.GetScalar(strMarks2));
                if (Marks2 == "" && IsGradeType == false)
                {
                    Marks2 = "0";
                }
                txtMarks3.Text = Marks2.ToString();
                dtSubjectMark.Rows[Convert.ToInt32(e.Row.RowIndex)][3] = Marks2.ToString();
                string strMarks3 = "Select "+ strSubMark + " from tbl_marks_master where sectionid=" + ddlsection.SelectedValue + " and examid=" + TempDt.Rows[Start3][0] + " and classid=" + ddlcourse.SelectedValue + " and subjectid=" + ddlsubject.SelectedValue + " and studentregno=" + lblregno.Text.ToString() + "";

                Marks3 = Convert.ToString(objDut.GetScalar(strMarks3));
                if (Marks3 == "" && IsGradeType == false)
                {
                    Marks3 = "0";
                }
                txtMarks4.Text = Marks3.ToString();
                dtSubjectMark.Rows[Convert.ToInt32(e.Row.RowIndex)][4] = Marks3.ToString();
                string strMarks4 = "Select "+ strSubMark + " from tbl_marks_master where sectionid=" + ddlsection.SelectedValue + " and examid=" + TempDt.Rows[Start4][0] + " and classid=" + ddlcourse.SelectedValue + " and subjectid=" + ddlsubject.SelectedValue + " and studentregno=" + lblregno.Text.ToString() + "";

                Marks4 = Convert.ToString(objDut.GetScalar(strMarks4));
                if (Marks4 == "" && IsGradeType == false)
                {
                    Marks4 = "0";
                }
        ...
        if (IsValid)
        {
            foreach (GridViewRow row in grdsetmarks.Rows)
            {
                ...
                if (Session["ExamTemp"] != null)
                {
                    ...
                    if (TempDt.Rows.Count > 0)
                    {
                        int result = uploadMarksNew(lblregno.Text, ddlsubject.SelectedValue, TempDt.Rows[Start1][0].ToString(), decmarks12.ToString(), IsGradeType);
                        int result1 = uploadMarksNew(lblregno.Text, ddlsubject.SelectedValue, TempDt.Rows[Start2][0].ToString(), decmarks13.ToString(), IsGradeType);
                        int result2 = uploadMarksNew(lblregno.Text, ddlsubject.SelectedValue, TempDt.Rows[Start3][0].ToString(), decmarks14.ToString(), IsGradeType);
                        int result3 = uploadMarksNew(lblregno.Text, ddlsubject.SelectedValue, TempDt.Rows[Start4][0].ToString(), decmarks15.ToString(), IsGradeType);
                ...
                if (!string.IsNullOrWhiteSpace(SubjectName))
                {
                    ...
                    if (dtSubjectId.Rows.Count > 0)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(SubjecTId))
                        {
                            ...
                            {
                                var mark = "";
                                string str = @"Select ISNULL(marks,'0') marks,ISNULL(Grade,'') Grade from tbl_marks_master where examid=" + examId + " and classid=" + ClassId +
                                             " and sectionid=" + sectionId + " and studentregno=" + StuReg + " and fyid=" + fyid +
                                             " and brid=" + Convert.ToInt64(Session["BrBrid"]) + " and subjectId=" + SubjecTId + " and status=1";
                                DataTable dtmark = objDut.GetDataTable(str);
                                string marks = "",grade="";
                                if (dtmark.Rows.Count > 0)
                                {
                                    marks = Convert.ToString(dtmark.Rows[0]["marks"]);
                                    grade = Convert.ToString(dtmark.Rows[0]["Grade"]);                                    
                                }
                                if (IsGrade)
                                {
                            ...
                            if (mode == 2)
                            {
                                ...
                                if (IsGrade)
                                {
                                    var isNumeric = !string.IsNullOrEmpty(oldMark) && oldMark.All(Char.IsDigit);
                                    if (!isNumeric)
                                    {
                                        value = Convert.ToString(oldMark);
                                    }
                                }
                                else
                                {
                                    decimal.TryParse(oldMark, out decmarks2);
                                    value = Convert.ToString(decmarks2);
                                }
                                ...
                                if (IsValidMark)
                                {
                                    uploadMarksNew(Convert.ToString(dtStd.Tables[0].Rows[i]["RegNo"]), SubjecTId, ddlExam.SelectedValue, value, IsGrade);
                                }
                            }
                        }
                    }
    ...
    public int uploadMarksNew(string regNo, string subjectId, string examId, string marks,bool isGrade)
    {
        int result = 0;
