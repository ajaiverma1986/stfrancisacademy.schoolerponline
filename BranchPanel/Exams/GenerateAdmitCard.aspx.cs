...
public partial class BranchPanel_Exams_GenerateAdmitCard : System.Web.UI.Page
{
    private string url;
    private int query;
    ...
    protected void btnVResult_Click(object sender, EventArgs e)
    {
        ...
        Session["AdmitFromDate"] = txtFromdate.Text;
        Session["AdmitToDate"] = txtTodate.Text;
        foreach (GridViewRow row in grdHouse.Rows)
        {
            CheckBox ChkResult1 = (CheckBox)row.FindControl("ChkResult1");
            ...
            if (ChkResult1.Checked == true)
            {
                ...
                dr["termid"] = ddlResultType.SelectedValue;
                dr["classid"] = ddlapplyingforclass.SelectedValue;
                dr["sectionid"] = ddlSection.SelectedValue;
                dtReg.Rows.Add(dr);
                dtReg.AcceptChanges();
            }
        }
