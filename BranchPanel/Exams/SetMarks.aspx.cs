...
public partial class BranchPanel_Exams_SetMarks : System.Web.UI.Page
{
    ...
    private void FillGrid(int classid, int sectionid, int SubjectID, int Mode)
    {
        ...
        if (classid > 0)
        {
            ...
            {
                StrQuery = " Order by Sr.StudentFirstName,Sr.StudentLastName asc";
            }
    ...
    private string getMarksFromDb(string examId, string regno, bool mode, bool IsGradeTypeSub = false, string subjectId = "")
    {
        ...
        string strMarks1 = @"Select ISnull(Grade,'') IsGrade, ISnull(marks,'0') marks, ISnull(IsAbsent,'0') IsAbsent from tbl_marks_master 
                            ...
                            " and Fyid =" + fyid + " and subjectid=" + subjectId + " and studentregno=" + regno.ToString() + " and brid=" + Convert.ToInt64(Session["BrBrid"]) + "";
        ...
        foreach (GridViewRow row in grdsetmarks.Rows)
        {
            ...
            if (Session["ExamTemp"] != null)
            {
                ...
                if (TempDt.Rows.Count > 0)
                {
                    ...
                    if (ResultType.Value == "1")
                    {
                        string strMarks1 = Convert.ToString(objDut.GetScalar("Select ISNULL(maxmarks,0) from exam_name where eid=" + TempDt.Rows[Start1][0] + " and fyid =" + fyid + " and brid=" + Convert.ToInt64(Session["BrBrid"]) + " and status=1"));
                        ...
                        string strMarks2 = Convert.ToString(objDut.GetScalar("Select ISNULL(maxmarks,0) from exam_name where eid=" + TempDt.Rows[Start2][0] + " and fyid =" + fyid + " and brid=" + Convert.ToInt64(Session["BrBrid"]) + " and status=1"));
    ...
    private void Bind_student_marks(int examid, int classid, int sectionid)
    {
        ...
        string strSubject = " Select DISTINCT Subject_Master.SubjectName,ISNULL(marks,0) marks from tbl_marks_master SEM " +
                            " inner join Subject_Master on Subject_Master.id = SEM.subjectid " + StrStr1 + " and SEM.Fyid =" + fyid + " and Subject_Master.SubjectName NOT IN('Health & Physical Education','Work Education','Discipline','Art Education')";
        DataTable dtSubject = objDut.GetDataTable(strSubject);
        if (dtSubject.Rows.Count > 0)
        {
            string strSubjectList = "";
    ...
    private void GetDataExcel(int classid, int sectionid)
    {
        ...
        if (classid > 0)
        {
            ...
            {
                StrQuery = " Order by Sr.StudentFirstName,Sr.StudentMiddleName,Sr.StudentLastName asc";
            }
                        ...
                        if (dtSubjectId.Rows.Count > 0)
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(SubjecTId))
                            {
                                ...
                                if (mode == 2)
                                {
                                    ...
                                    {
                                        decimal Marks5 = 0;
                                        string strMarks5 = Convert.ToString(objDut.GetScalar("Select ISNULL(maxmarks,0) from exam_name where eid=" + examId + " and fyid =" + fyid + " and brid=" + Convert.ToInt64(Session["BrBrid"]) + " and status=1"));
                                        if (strMarks5.ToString() == "")
                                        {
                                            strMarks5 = "0";
                                        }
        ...
        if (ddlUpdateOption.SelectedIndex > 0)
        {
            ...
            else
            {
                ...
                if (ddlTerm.SelectedValue == "1")
                    ...
                    strSUbQuery = "desc";
                DataTable dtExam = objDut.GetDataTable(@"Select distinct TOP 2 examid,examName from exam_class " +
                                                        "Inner  join exam_name on eid = examid " +
                                                        " where classid = " + ddlcourse.SelectedValue + " and exam_class.brid=" + Convert.ToInt64(Session["BrBrid"]) + " and exam_class.status=1 order by examid " + strSUbQuery);
