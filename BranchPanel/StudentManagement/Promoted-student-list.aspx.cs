...
public partial class BranchPanel_StudentManagement_Promoted_student_list : System.Web.UI.Page
{
   ...
   public void BindSessionDDL()
    ...
    public static studentregistration[] BindGridByJs(string Fromdate, string Todate, string Registrationnumber, string StudentName, string Parentname, string ParentMobileNo, int Status, int Applyingforclass, int Fyid, string Admissionnnumber)
    {
        ...
        using (SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"].ToString()))
        {
            using (SqlCommand cmd = new SqlCommand("[usp_studentadmisionlist]", con))
            {
                ...
                param[2] = new SqlParameter("@IsPromoted", 1);
                ...
                if (ds.Tables.Count > 0)
                {
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        foreach (DataRow dtrow in ds.Tables[0].Rows)
                        {
                            ...
                            user.Promoteclass = dtrow["classname"].ToString();
                            user.PromoteSecName = dtrow["SectionName"].ToString();
                            user.PromoteclassID = dtrow["ClassID"].ToString();
                            user.PromoteSecNameID = dtrow["SectionID"].ToString();
                            details.Add(user);
                        }
                    }
                }
    ...
    public class studentregistration
    {
        ...
        public string PrimaryMobileNo { get; set; }
        public string Promoteclass { get; set; }
        public string PromoteSecName { get; set; }
        public string PromoteclassID { get; set; }
        public string PromoteSecNameID { get; set; }
    }
}