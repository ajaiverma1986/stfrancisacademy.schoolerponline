...
public partial class BranchPanel_Certificates_CharacterCertificate : System.Web.UI.Page
{
    ...
    public void Update(int ID)
    {
        ...
        StudentName,ISNULL(SR.PrimaryParentFirstName,'')+' '+ISNULL(SR.PrimaryParentMiddleName,'')+' '+ISNULL(SR.PrimaryParentLastName,'') AS 
        ParentName,SR.StudentDOB, CM.Classname,FM.FYName from tbl_StudentRegistration SR
        inner join tbl_StudentMaster SM on SM.RID = SR.RID
        inner join Class_Master CM on cm.id = SR.ApplyingForClass
        inner join financial_Year FM on FM.fyid = SR.Fyid where SM.SturegNo=" + ID + " and SR.Brid=" + Convert.ToInt32(Session["BrBrid"]) + " and SM.IsActive=1";

        dtstd = Objdut.GetDataTable(Usertype);
        if (dtstd.Rows.Count > 0)
        {
            txtName.Text = dtstd.Rows[0]["StudentName"].ToString();
            //lblclass.Text = dtstd.Rows[0]["Classname"].ToString();
            txtBdate.Text = Convert.ToDateTime(dtstd.Rows[0]["StudentDOB"]).ToString("dd-MMM-yyyy");
           // lbldobw.Text = WrittenNumerics.DateToWritten(Convert.ToDateTime(dtstd.Rows[0]["StudentDOB"])).ToString().ToUpper();
            //lblFyid.Text = dtstd.Rows[0]["FYName"].ToString();
            txtFName.Text = dtstd.Rows[0]["ParentName"].ToString();
        }
    }
}