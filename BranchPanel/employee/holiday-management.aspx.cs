...
public partial class BranchPanel_holiday_management : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static int InsertHoliday(string year, string month, string day, string reason)
    {
        using (SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["ConnectionString"].ToString()))
        {
            using (SqlCommand cmd = new SqlCommand("[usp_Insert_Holiday]", con))
            {
                ...
                param[0] = new SqlParameter("@year", fyid);
