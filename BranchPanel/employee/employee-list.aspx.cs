...
public partial class BranchPanel_employee_list : System.Web.UI.Page
{
    ...
    private void Get_Emp_List()
    {
        ...
        DataTable Dt = objdut.GetDataTableSP(param, "[usp_Get_Emp_List]");
        lblTotal.Text = Dt.Rows[0]["Total"].ToString();
        lblworking.Text = Dt.Rows[0]["Working"].ToString();
        lblsupended.Text = Dt.Rows[0]["Suspend"].ToString();
        lblonleave.Text = Dt.Rows[0]["On_Leave"].ToString();
        lblnoticeperiod.Text = Dt.Rows[0]["Notice_period"].ToString();
    }
    ...
    [WebMethod]
    public static MarkAsRed[] GEtRedMarkEmployeeList()
    {
        DataTable Dt = new DataTable();
        List<MarkAsRed> details = new List<MarkAsRed>();
        DataUtility ObjDUT = new DataUtility();
        Dt = ObjDUT.GetDataTableSP("[usp_RedMark_WhoseProbationPeriodComplete]");
        for (int i = 0; i < Dt.Rows.Count; i++)
        {
            MarkAsRed user = new MarkAsRed();
            user.RowNumber = Dt.Rows[i]["RowNumber"].ToString();
            user.eid = Dt.Rows[i]["eid"].ToString();
            user.refNo = Dt.Rows[i]["refno"].ToString();
            details.Add(user);
        }
        return details.ToArray();
    }

    public class MarkAsRed
    {
        public string RowNumber { get; set; }
