...
public partial class BranchPanel_Fee_Management_DailyFeeTrans : System.Web.UI.Page
{
    ...
    [WebMethod]
    public static GetDailyFeeDetails[] BindGridOfDailyFeeList(int Fyid, string pagesize, string pageno, string stuname, string duefromdate, string duetodate, string Paidfromdate, string Paidtodate, string mobileno, string classid)
    {
        ...
        param[9] = new SqlParameter("@Paidfromdate", Paidfromdate);
        param[10] = new SqlParameter("@PaidToDate", Paidtodate);
        ...
        if (Ds.Tables[0].Rows.Count > 0)
        {
            foreach (DataRow Dtrow in Ds.Tables[0].Rows)
            {
                ...
                user.PaidDate = Convert.ToDateTime(Dtrow["PaidDate"]).ToString("dd MMM yyyy");
