...
public partial class BranchPanel_MasterUC_ExamName : System.Web.UI.Page
{
    ...
    {       
        ...
        Dt = CommonClass.ABC("GetClassDDL", Convert.ToInt32(objCommon.GetActiveAcedmicYear()), Convert.ToInt32(Session["BrBrid"]));
        if (Dt.Rows.Count > 0)
        {
    ...
    {
        string querynew = @"Select HouseID,Housename from mst_House where Brid=" + Session["BrBrid"] + " and isActive=1 and Fyid=" + Convert.ToInt32(objCommon.GetActiveAcedmicYear()) + "order by HouseID asc";
    ...
    public void Section()
    {
        ...
        string querynew = @"SELECT DISTINCT ISNULL(cs.cwsid,0) SectionID,ISNUll(cs.sectionname,'') SectionName from Class_Master SM
                            inner join ClassBranch_master CBM on CBM.classid = SM.id
                            inner join classwithsection cs on cs.classid = CBM.classid and cs.branchid = CBM.BranchId
                            where SM.id=" + ddlapplyingforclass.SelectedValue + " and cs.branchid=" + Session["BrBrid"] + " and SM.Fyid = " + fyid + " and cs.status=1 order by sectionid asc";
    ...
    protected void grdHouse_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if (ViewMode == "")
            {
                ddlHouse.DataSource = CommonClass.Dll("GetHouse", Convert.ToInt32(Session["BrBrid"]), fyid);
    ...
    public void FillStudent(int HouseID)
    {
        ...
        string querynew = @"SELECT DISTINCT ISNULL(SA.ClassID,0) Class,ISNULL(SA.SectionId,0) Sec,ISNULL(MH.Housename,'') HouseName,ISNULL(SM.StuRegNo,0) StuRegNo,ISNULL(SA.AdmissionNo,'') AdmissionNo,ISNULL(SR.HouseID,0) HouseID ,
                           ...
                           from tbl_StudentRegistration SR
                            ...
                            where SR.Brid=" + Session["BrBrid"] + " and SR.fyid = " + fyid + querynew1;
