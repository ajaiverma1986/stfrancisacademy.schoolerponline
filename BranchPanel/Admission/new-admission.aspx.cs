...
public partial class BranchPanel_Admission_new_admission : System.Web.UI.Page
{
    ...
    protected void btnforupdateorsave_Click(object sender, EventArgs e)
    {
        ...
        if (Count == 0)
        {
            ...
            if (intresult == 1)
            {
                ...
                string RegNo = objsave.updateORinsertHostelFee(txtregistrationno.Text.Trim(), Convert.ToInt32(Session["BrBrid"]), txtstudentfirstname.Text.Trim(), Convert.ToInt32(rbtnlistHostal.SelectedValue)).ToString();
                int RIDNEW = Convert.ToInt32(Objdut.GetScalar("Select isnull(rid,0) from tbl_StudentRegistration where  Registration_No='" + RegNo + "' and Brid=" + Convert.ToInt32(Session["BrBrid"]) + ""));
                ...
                Response.Redirect("admission-list.aspx");
                return;
            }
            else if (intresult == 2)
            {
